...
first_long|>
  ...
  theme_classic()+
  geom_vline(xintercept = mean(first_long$pass_rate), linetype= "dashed")+
  geom_hline(yintercept= mean(first_long$epa_play), linetype= "dashed")+
  labs(
    x= "Pass Rate",
    y= "EPA Per Play",
    title= "EPA Per Play on First Downs of more then 7 Yards",
    subtitle= "2022 Regular Season")
ggsave("First-Long.png", width= 14, height= 10, dpi= "retina")
